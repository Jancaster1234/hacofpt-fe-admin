...
const ChatDetails: React.FC<ChatDetailsProps> = ({
  ...
  const [activeReactionBar, setActiveReactionBar] = useState<string | null>(
    null
  );
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {chat.messages.map((message) => {
          const isCurrentUser =
            ...
            user?.username &&
            message.createdByUserName === user.username;
          // Tooltip for full date/time
          const fullDate = formatTime(message.createdAt);
          return (
            <div
              key={message.id}
              className={`flex w-full ${isCurrentUser ? "justify-end" : "justify-start"} items-end`}
            >
              {/* Avatar for others */}
              {!isCurrentUser && (
                <img
              ...
              <div
                className={`flex flex-col max-w-[70%] ${isCurrentUser ? "items-end ml-auto" : "items-start mr-auto"} relative group`}
              >
                {/* Name for others */}
                {!isCurrentUser && (
                ...
                <div className="flex items-center w-full">
                  ...
                  {isCurrentUser && (
                    <button
                      ...
                      onClick={() =>
                        setActiveReactionBar(
                          activeReactionBar === message.id ? null : message.id
                        )
                      }
                      tabIndex={-1}
                    >
                ...
                {activeReactionBar === message.id && (
                  <div
                    className={`absolute ${isCurrentUser ? "right-0" : "left-0"} -bottom-7 z-40`}
                  >
