// src/components/data-table/data-table-form.tsx
"use client";
    ...
    <Form {...form}>
      <form className={"space-y-2"} onSubmit={form.handleSubmit(onSubmit)}>
        {schemas.map((schemaProp, index) => {
          const { id, component, label, description } = schemaProp;
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  {!_.isEmpty(label) && (
                    <FormLabel
                      className={"uppercase text-xs text-slate-500 font-bold"}
                    >
                      {label}
                    </FormLabel>
                  )}
  ...
  if (component === "input") {
    return (
      <Input
        ...
        className={componentCssProps?.parent}
        {...formFieldProps}
      />
    );
  }
  if (component === "select" && formFieldProps) {
    return (
      <Select onValueChange={formFieldProps.onChange} {...formFieldProps}>
        ...
        <SelectContent>
          <SelectGroup>
            {!_.isEmpty(label) && <SelectLabel>{label}</SelectLabel>}
            {!_.isEmpty(data) &&
              data?.map(({ value, children }, index) => (
                <SelectItem
  ...
  if (component === "combobox" && formFieldProps && formProps) {
    ...
    return (
      <Popover open={inStateOpen} onOpenChange={setInStateOpen}>
        ...
        <PopoverContent
          className={componentCssProps?.parent || "w-[180px] p-0"}
        >
          <Command>
                ...
                render={({ field }) => {
                  return (
                    ...
                    >
                      ...
                      <FormLabel
                        className={
                          componentCssProps?.child || "text-sm font-normal"
                        }
                      >
                        {item.children}
                      </FormLabel>
