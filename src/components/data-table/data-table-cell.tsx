// src/components/data-table/data-table-cell.tsx
"use client";
...
export function DataTableCell<T>({ cell }: IDataTableCellEdit<T>) {
  ...
  const { contextMenuProps, isSelecting } = useDataTableStore((state) => ({
    ...state,
  }));
  ...
  const showContextMenu =
    isSelecting !== true && contextMenuProps !== undefined;
  if (showContextMenu) {
    return (
        ...
        <ContextMenu>
          ...
          <ContextMenuContent className="w-36">
            {contextMenuProps.enableEdit && (
              <DataTableEditRow
                presetData={cell.row.original as { [k: string]: any }}
              />
            )}
                ...
                <ContextMenuSubContent className="w-48">
                  {Object.keys(contextMenuProps?.extra).map((name, index) => (
                    <ContextMenuItem
                      ...
                      key={"Sub-data-table-context-menu-item-".concat(
                        index.toString()
                      )}
                    >
                      {name}
