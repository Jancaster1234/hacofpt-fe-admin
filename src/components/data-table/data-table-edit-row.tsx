// src/components/data-table/data-table-edit-row.tsx
"use client";
...
export function DataTableEditRow<T>({
  presetData,
}: {
  presetData: { [k: string]: any };
}) {
  const { title, description, onSubmitEditData, schemas } = useDataTableStore(
    (state) => ({
      ...state.editDataProps,
      schemas: state.editDataValidationProps,
    })
  );
  const getFormSchema = () => {
    ...
    if (schemas instanceof Array && schemas.length > 0) {
      ...
      schemas.forEach((item) => {
        ...
        defaultValues[item.id] =
          item.id in presetData ? presetData[item.id] : "";
      });
  ...
  return (
    <Sheet>
      ...
      >
        <ContextMenuNotItem>Edit Row</ContextMenuNotItem>
      </SheetTrigger>
      <SheetContent className={"space-y-2 overflow-y-auto"}>
        ...
        <Separator />
        {Object.keys(FormSchema.defaultValues).length > 0 &&
          schemas !== undefined && (
